...
public class NXJFlashUpdate
{
     ...
     */
    public byte[] createFirmwareImage(String vmName, String menuName) throws IOException, FileNotFoundException
    {
    ...
    /**
     * Locate and open an nxt device in SAM-BA mode. If none are present wait
     * up to timeout ms checking to see if one has become available.
     * @return
     */
     ...
     * @param nxt device to verify
     * @param address starting address
     * @param memoryImage memory address to compare with
     * @return number of mismatched bytes found
     * @throws java.io.IOException
     */
    public int verifyPages(NXTSamba nxt, int address, byte[] memoryImage) throws IOException
    {
        ...
        for(int page = 0; page < pages; page++)
        {
            ...
            for(int i = 0; i < actual.length; i++)
            {
                ...
                {
                        ui.message(String.format("Verify failed at address 0x%08X expected 0x%02X found 0x%02X\n", (address*NXTSamba.PAGE_SIZE + offset), (memoryImage[offset] & 0xff), (actual[i] & 0xff)));
                    failCnt++;
                }
                offset++;
            }
    ...
    public void writePages(NXTSamba nxt, int address, byte[] memoryImage) throws IOException
    {
        int pages = memoryImage.length/NXTSamba.PAGE_SIZE;
        for(int page = 0; page < pages; page++)
        {
            ui.progress("Writing", (page*100/pages));
            nxt.writePage(address+page, memoryImage, page*NXTSamba.PAGE_SIZE);
        }
        ui.progress("", 0);
    }
     ...
     * @param nxt Device to format
     * @param fs File system image to use
     * @throws java.io.IOException
     */
     ...
     * @param nxt device to verify
     * @param image firmware image to compare against
