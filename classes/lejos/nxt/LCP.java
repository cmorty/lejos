...
public class LCP {
	...
	public static void emulateCommand(byte[] cmd, int cmdLen)
	{
		...
		if (cmd[1] == 0x07) {
			byte port = cmd[2];
			int raw = Port.PORTS[port].readRawValue();
			int scaled = Port.PORTS[port].readValue();
			int norm = 1024 - raw;
			
			reply[3] = port;
			reply[4] = 1;
			reply[5] = 0;
			reply[6] = (byte) Port.PORTS[port].getType();
			reply[7] = (byte) Port.PORTS[port].getMode();
			reply[8] = getLSB(raw);
			reply[9] = getMSB(raw);
		...
		if (cmd[1] == 0x0F)
		{
			...
			byte txLen = cmd[3];
			byte rxLen = cmd[4];
			Port.i2cEnableById(port);
			try {Thread.sleep(100);} catch(InterruptedException ie) {}
			int ret = Port.i2cStartById(port, cmd[5] >> 1, cmd[6], rxLen, i2cReply, rxLen, 0);

			while (Port.i2cBusyById(port) != 0) {
				Thread.yield();
			}
			try {Thread.sleep(100);} catch(InterruptedException ie) {}
			i2cLen = rxLen;
