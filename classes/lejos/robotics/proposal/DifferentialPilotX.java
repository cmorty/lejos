...
/**
 * The DifferentialPilotX class is a software abstraction of the Pilot mechanism of a
 ...
 * addPilotListener() method on this class.
 ...
 * <p>
 * <code><pre>
 * PdifferentialPilot pilot = new DifferentialPilotX(2.1f, 4.4f, Motor.A, Motor.C, true);  // parameters in inches
 ...
 * 
 **/
public class DifferentialPilotX implements Pilot
{
	 ...
	 */
	public DifferentialPilotX(final float wheelDiameter, final float trackWidth,
	 ...
	 *            running backward.
	 */
	public DifferentialPilotX(final float leftWheelDiameter,
			final float rightWheelDiameter, final float trackWidth,
			final TachoMotor leftMotor, final TachoMotor rightMotor,
			final boolean reverse)
    {
      _left = (Motor)leftMotor;
    ...
    public void addPilotListener(PilotListenerX aListener)
    {
...
//    }
	...
	public void travelArc(float radius, float distance, boolean immediateReturn) {
		...
		arc(radius, (float)angle, immediateReturn); 
	}
	/**
    ...
    {
      for(PilotListenerX p : listeners)
        {
 ...
 public Movement getMovement()
      ...
      private class Monitor extends Thread
    {
      public void run()
      {
        ...
        while(!_alert)Thread.yield();//no movement in progress
       while(!isMoving()&&_alert)Thread.yield();//wait till started
        while(isMoving() && _alert)Thread.yield(); //  now it is done
        if(isMoving() &&_alert) stop();
         _alert = false;
      }
    }
/**
 ...
 */
     ...
     * the pilot listeners
     */
    protected ArrayList<PilotListenerX> listeners = new ArrayList<PilotListenerX>();
    /**
