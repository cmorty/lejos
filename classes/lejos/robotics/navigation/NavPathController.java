...
/**
 * This class can cause the robot to follow a route - a sequence of
 * {@link lejos.robotics.navigation.WayPoint } ; The way points are stored in a
 * queue (actually, a Collection). This class uses an inner class running its
 * own thread to issue movement commands to its
 * {@link lejos.robotics.navigation.MoveController}, which can be either a
 * {@link lejos.robotics.navigation.DifferentialPilot} or
 * {@link lejos.robotics.navigation.SteeringPilot}. It also uses a
 * {@link lejos.robotics.localization.PoseProvider} to keep its pose updated,
 * and calls its {@link lejos.robotics.navigation.WayPointListener} when a way
 * point is reached.
 * 
...
public class NavPathController implements PathController {
	...
	/**
	 * Creates a PathController using a custom poseProvider, rather than the
	 * default OdometryPoseProvider.
	 * 
	 * @param pilot
	 * @param poseProvider
	 */
	public NavPathController(MoveController pilot, PoseProvider poseProvider) {
		...
		_radius = (_pilot instanceof ArcMoveController ? ((ArcMoveController) _pilot)
				.getMinRadius() : 0);
		_nav = new Nav();
		_nav.setDaemon(true);
		_nav.start();
	...
	/**
	 * Creates a PathController which will navigate to a point via goTo() using
	 * a PathFinder to provide assistance to create a path.
	 * 
	 * @param pilot
	 * @param poseProvider
	 * @param pathFinder
	...
	public void setPathFinder(PathFinder pathFinder) {
		...
		final PathController pc = this; // Grab reference to NavPathController
		...
		pathFinder.addListener(new WayPointListener() {
			...
			public void pathComplete() {
				// Nothing to do. PathController keeps waiting for more
				// Waypoints in queue.
			}
		});
	}
	...
	/**
	 * returns <code> false </code> if the the final waypoint has been reached
	 * or interrupt() has been called
	 */
	...
	public void goTo(WayPoint destination) {
		...
		// TODO: It would be helpful to return boolean if it got to destination
		// successfully.
	}
	 ...
	 * queue and then stop. Meanwhile, isGoing() returns true. This method
	 * returns immediately.
	 */
	...
	public void addTargetListener(WayPointListener targetListener) {
		if (targetListeners == null)
			targetListeners = new ArrayList<WayPointListener>();
	 ...
	 * @return the waypoint
	 */
	public WayPoint getWayPoint() // TODO: Delete this method? Or add to
									// PathController interface? Might be used
									// by some other sample?
	{
		if (_route.size() <= 0)
			return null;
		return _route.get(0);
	...
	public void waitForDestinationReached() { // TODO: Delete this method? Might
												// be used by some other sample?
		while (_keepGoing)
			Thread.yield();
	}
		...
		public void run() {
			while (more) {
				while (_keepGoing)// && _route != null && _route.size()>0)
				{
					...
					float destinationRelativeBearing = _pose
							.relativeBearing(_destination);
					if (!_keepGoing)
						break;
					if (_radius == 0) {
						...
						// 1. Get shortest path:
						Move[] moves;
						double minRadius = (_pilot instanceof ArcMoveController ? ((ArcMoveController) _pilot)
								.getMinRadius() : 0);

						if (_destination.headingRequired) {
							moves = ArcAlgorithms.getBestPath(
									poseProvider.getPose(), (float) minRadius,
									_destination.getPose(), (float) minRadius);
						} else {
							moves = ArcAlgorithms.getBestPath(
									poseProvider.getPose(), _destination,
									(float) minRadius);
						}
						// 2. Drive the path
						for (int i = 0; i < moves.length; i++) {
							((ArcMoveController) _pilot).travelArc(
									moves[i].getArcRadius(),
					...
					if (_radius == 0) {
						...
						if (_destination.headingRequired) {
							...
							((RotateMoveController) _pilot).rotate(_destination
									.getHeading() - _pose.getHeading());
						}
					}
