...
/**
 * J2ME Bluetooth implementation of NXTComm
 * 
 ...
 *
 */
public class NXTCommBluetooth implements NXTComm, DiscoveryListener {
	private static Vector devices;
    private static Vector nxtInfos;
	...
	public NXTInfo[] search(String name, int protocol) throws NXTCommException {
		...
		synchronized (this) {
			try {
				...
				throw new NXTCommException("Bluetooth stack not detected"); 
			}
		}
	...
	public boolean open(NXTInfo nxt, int mode) throws NXTCommException {
		...
		try {
			...
			nxt.connectionState = NXTInfo.CONNECTED;
			return true;
		} catch (IOException e) {
			nxt.connectionState = NXTInfo.DISCONNECTED;
