 ...
 * 
 * This module implements two types of I/O over USB. 1. The standard Lego LCP
 * format used for LCP command processing. 2. A Simple packet based protocol
 * that can be used to transport a simple byte stream. Protocol 2 is required
 * (rather then using raw USB operations), to allow the signaling of things like
 * open, and close over the connection.
 * 
 * Notes This module assumes that the device read and write functions have a
 * built in timeout period of approx 20 seconds. This module assumes that this
 * timeout exists and uses it to timeout some requests.
 * 
 * Should not be used directly - use NXTCommFactory to create an appropriate
 * NXTComm object for your system and the protocol you are using.
 * 
 */
public abstract class NXTCommUSB implements NXTComm {
	...
	/**
	 * Return a vector of available nxt devices. Each NXTInfo item should have
	 * the address field populated and the other fields must contain sufficient
	 * information such that a call to devIsValid will return true and that
	 * devOpen will connect to the device. The name field may be left empty, in
	 * which case it will be populated by code in this class.
	 * 
	 ...
	 *            Device to read from.
	 * @param data
	 *            Location to place the read bytes.
	 * @param offset
	 *            Offset of where to place the bytes.
	 * @param len
	...
	/**
	 * Test to see if the contents of the NXTInfo structure are sufficient to
	 ...
	 * 
	 * @param addr
	 *            The adrress containing the string
	 * @param loc
	 ...
	 * 
	 * @param nxt
	 *            the device to obtain the name for
	 * @return the nxt name.
	 */
	private String getName(NXTInfo dev) {
		...
		if (nxt == 0)
			return name;
		byte[] request = { NXTProtocol.SYSTEM_COMMAND_REPLY,
				NXTProtocol.GET_DEVICE_INFO };
	 ...
	 * 
	 * @param block
	 *            true if request should block rather than timeout
	...
	/**
	 * Helper function. Write a single packet. Handles packet format and request
	 * timeouts.
	 * 
	 * @param data
	 * @param offset
	 * @param len
	 * @param block
	 *            true if requests should block rather than timeout
	 * @return number of bytes actually written
	 * @throws java.io.IOException
	 */
	private int writePacket(byte[] data, int offset, int len, boolean block)
			throws IOException {
		byte[] out = null;
		if (packetMode) {
			if (len > USB_BUFSZ - 1)
	 ...
	 * 
	 * @param nxtNames
	 *            an array of device address strings.
	 * @return
	 */
	Vector<NXTInfo> find(String[] nxtNames) {
		...
		for (int idx = 0; idx < nxtNames.length; idx++) {
			...
			// this device.
			if (info.deviceAddress != null
					&& !info.deviceAddress.equals("000000000000"))
	 ...
	 * 
	 * @param name
	 *            The name to search for. If null return all devices.
	 * @param protocol
	...
	public NXTInfo[] search(String name, int protocol) {
		...
		while (devs.hasNext()) {
			...
			if (name != null && !name.equals(nxt.name))
				...
				System.out.println("Found nxt name " + nxt.name + " address "
						+ nxt.deviceAddress);
		}
	...
	public boolean open(NXTInfo nxtInfo, int mode) {
		...
		// 0x02, 0xfe, 0xef
		if (ret != null && ret.length >= 3 && ret[0] == 0x02
				&& ret[1] == (byte) 0xfe && ret[2] == (byte) 0xef)
			packetMode = true;
		EOF = false;
		return true;
	}
