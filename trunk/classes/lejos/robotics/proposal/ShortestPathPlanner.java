...
public class ShortestPathPlanner
{
  ...
  */
  public ArrayList<WayPoint> findPath(Point start, Point finish, ArrayList<Line> theMap )
  {
    ...
    Node from = source;
    Node dest =  _candidate.get(_candidate.size()-1);
    boolean failed = false;
    while (_candidate.size() > 0 && !failed)
    {
      dest =  _candidate.get(_candidate.size()-1);// top of stack
      from = getBest(dest);
      if(from.getDistance(dest) >= BIG) failed = true ;
//     if blocked, side effect may add  two nodes to Candidate stack
      if( segmentBlocked(from, dest))  // set by segmentBlocked
      {
      ...
      } else  // not blocked
      {
        ...
        if (_reached.indexOf(dest) == -1) // dest not in _reached
        {
          _reached.add(dest); // add only if not in the set
        }
        _candidate.remove(dest);  // pop the stack
      }
    }    // candidate stack is empty - route complete

    if(failed)return null;
    else  return getRoute(destination);
  }
   ...
   * Side effect: nodes at the end of the blocking line are added to the candidate set
  ...
  protected boolean segmentBlocked(final Node from, final Node theDest)
  {
    ...
    Line segment = new Line(from.getX(), from.getY(),
            to.getX(), to.getY());
    for (Line l : _map)// test ever line in th emap to see if it blocks the segment
    {
   ...
   */
  protected boolean alreadyReached(final Node  aNode)
  {
    boolean found = false;
    for (Node n : _reached)
    {
