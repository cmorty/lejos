...
/**
 * The Pilot class is a software abstraction of the Pilot mechanism of a NXT robot. It contains methods to control robot
 * movements: travel forward or backward in a straight line or a circular path or rotate to a new direction.<br>
 * Note: this class will only work with two independently controlled Pilot motors to steer differentially, so it can
...
public class Pilot {
   ...
   * @param reverse If true, the NXT robot moves forward when the motors are running backward.
   */
  public Pilot(float wheelDiameter, float trackWidth, float driftCorrection, Motor leftMotor, Motor rightMotor,
      boolean reverse) {
